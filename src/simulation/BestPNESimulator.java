		...
		for(int n = MIN_NUM_PLAYERS ; n <= MAX_NUM_PLAYERS; n += NUM_PLAYERS_STEP){
			...
			for(int i = 0 ; i < REPEAT_COUNT; i++){
				...
				try {
					...
					writeToCSV("./stats-best-pne/ga1-gen/big-graph/players-"+n +"/iter-"+ i +  ".csv", stat1);
					...
					writeToCSV("./stats-best-pne/ga2-gen/big-graph/players-"+n +"/iter-"+ i +  ".csv", stat2);
...
//					iterSolQuality[3] = e.evaluate(g, sol4);
			...
			writeToCSV("./stats-best-pne/run-time/big-graph/players-"+n+".csv", runTime);
			writeToCSV("./stats-best-pne/sol-quality/big-graph/players-"+n+".csv", solQuality);
		}
